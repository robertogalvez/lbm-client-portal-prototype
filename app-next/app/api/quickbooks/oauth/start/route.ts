// Begin Intuit OAuth 2.0 authorization for QuickBooks Online. Admin-only.
// Generates a CSRF `state`, stashes it in a short-lived httpOnly cookie, and
// redirects to Intuit's consent screen. The callback completes the exchange
// (confidential client — client_secret via HTTP Basic, no PKCE).
...
export async function GET(req: Request) {
  ...
  res.cookies.set('quickbooks_oauth', JSON.stringify({ state }), {
    httpOnly: true,
    secure: true,
    sameSite: 'lax',
    path: '/',
