...
function StatusBadge({ status }: { status: string }) {
  const color = STATUS_COLORS[norm(status)] ?? '#8d8d8d';
  return (
    <span style={{ background: color + '22', color, fontSize: 11, fontWeight: 600, padding: '2px 8px', borderRadius: 4, whiteSpace: 'nowrap' }}>
      {status}
    </span>
  );
}
...
const MINI_PIPELINE = [
  { key: 'not ready',                 color: '#8d8d8d' },
  { key: 'backlog',                   color: '#8d8d8d' },
  { key: 'in progress (editor)',      color: '#1090e0' },
  { key: 'in progress (corrections)', color: '#1090e0' },
  { key: 'tc - qc (somu)',            color: '#7C4DFF' },
  { key: 'qc final - am',             color: '#7C4DFF' },
  { key: 'for client review',         color: '#ffc53d' },
  { key: 'ready to be posted',        color: '#20c997' },
  { key: 'posted in socials',         color: '#30a46c' },
];
...
export default async function AmPage() {
  ...
  if (!amName) return <AmPicker members={allAmNames} />;
  ...
  type ClientData = { active: number; postedThisMonth: number; inReview: number; oldestReviewMs: number; pipeline: Record<string, number> };
  const clientMap = new Map<string, ClientData>();
  for (const t of amTasks) {
    const cn = t.clientName ?? 'Unknown';
    if (!clientMap.has(cn)) clientMap.set(cn, { active: 0, postedThisMonth: 0, inReview: 0, oldestReviewMs: 0, pipeline: {} });
    const cd = clientMap.get(cn)!;
    const ns = norm(t.status);
    cd.pipeline[ns] = (cd.pipeline[ns] ?? 0) + 1;
    if (ns !== POSTED_NORM) cd.active++;
    if (ns === POSTED_NORM && new Date(t.dateUpdated).getTime() >= monthStart) cd.postedThisMonth++;
  ...
  const attentionTasks = amTasks.filter(t => {
    const ns = norm(t.status);
    return ns === 'qc final - am' || (ns === 'for client review' && daysWaiting(t.dateUpdated) > 2);
  ...
  return (
    <main style={{ padding: '28px 32px', maxWidth: 1400 }}>
      <div style={{ marginBottom: 24 }}>
        <h1 style={{ fontSize: 24, fontWeight: 700, color: '#111c28', margin: 0 }}>{greeting(amName)}</h1>
        <p style={{ fontSize: 13, color: '#8b97a4', margin: '4px 0 0' }}>Your client workload · Live from ClickUp</p>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(320px, 1fr))', gap: 16, marginBottom: 28 }}>
        {clientEntries.map(([clientName, cd]) => {
          ...
          return (
            <div key={clientName} style={{ background: '#fff', border: '1px solid #e7ebef', borderRadius: 12, padding: '20px' }}>
              <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 14 }}>
                <div style={{ fontWeight: 700, fontSize: 15, color: '#111c28' }}>{clientName}</div>
                {needsAttn && <span style={{ fontSize: 11, fontWeight: 600, color: '#e59700', background: '#fef4e0', padding: '2px 8px', borderRadius: 4 }}>Needs Attention</span>}
              </div>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 12, marginBottom: 14 }}>
                <div><div style={{ fontSize: 11, color: '#8b97a4', marginBottom: 2 }}>Active</div><div style={{ fontSize: 18, fontWeight: 700, color: '#111c28' }}>{cd.active}</div></div>
                <div><div style={{ fontSize: 11, color: '#8b97a4', marginBottom: 2 }}>Posted (mo)</div><div style={{ fontSize: 18, fontWeight: 700, color: '#30a46c' }}>{cd.postedThisMonth}</div></div>
                <div><div style={{ fontSize: 11, color: '#8b97a4', marginBottom: 2 }}>In Review</div><div style={{ fontSize: 18, fontWeight: 700, color: cd.inReview > 0 ? '#ffc53d' : '#111c28' }}>{cd.inReview}</div></div>
              </div>
      ...
      <div style={{ background: '#fff', border: '1px solid #e7ebef', borderRadius: 10, overflow: 'hidden', marginBottom: 28 }}>
        {attentionTasks.length === 0 ? (
          <div style={{ padding: '24px', textAlign: 'center', fontSize: 14, color: '#8b97a4' }}>Nothing needs attention right now.</div>
        ) : (
            ...
            <tbody>
              {attentionTasks.map(t => (
                <tr key={t.clickupTaskId} style={{ borderBottom: '1px solid #f4f6f8' }}>
                  <td style={{ padding: '10px 16px', fontWeight: 500, color: '#111c28', maxWidth: 300 }}><div style={{ overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{t.title}</div></td>
                  ...
                  <td style={{ padding: '10px 16px', fontWeight: 700, color: daysWaiting(t.dateUpdated) > 3 ? '#e5484d' : '#111c28' }}>{daysWaiting(t.dateUpdated)}d</td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      <div style={{ background: '#fff', border: '1px solid #e7ebef', borderRadius: 10, overflow: 'hidden' }}>
        {dueTasks.length === 0 ? (
          <div style={{ padding: '24px', textAlign: 'center', fontSize: 14, color: '#8b97a4' }}>No tasks due in the next 7 days.</div>
        ) : (
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
            <thead>
              <tr style={{ borderBottom: '1px solid #e7ebef', background: '#f8f9fb' }}>
