    ...
    <div style={{ background: '#fff', border: '1px solid #ece4d8', borderRadius: 22, overflow: 'hidden' }}>
      ...
      <div style={{
        ...
        <span style={{
          ...
          color: '#fff', fontSize: 10, fontWeight: 700, padding: '3px 8px', borderRadius: 7, letterSpacing: '0.02em',
        }}>Frame.io</span>
        <div style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' as const, fontSize: 12, color: '#9d9488', fontWeight: 500 }}>
          {task.assignedAmName && (
            <span style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
              ...
              {task.assignedAmName.split(' ')[0]} {task.assignedAmName.split(' ')[1]?.[0]}.
            </span>
          )}
          <span style={{ width: 3, height: 3, borderRadius: '50%', background: '#9d9488' }} />
          <span>{waiting === 0 ? 'Today' : waiting === 1 ? 'Yesterday' : `${waiting}d ago`}</span>
          {task.dueDate && (
            <><span style={{ width: 3, height: 3, borderRadius: '50%', background: '#9d9488' }} /><span>Due {fmtDate(task.dueDate)}</span></>
          )}
        </div>
